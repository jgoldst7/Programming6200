...
FinalBioIO::MyIO - package to handle opening of files and passing filehandles

=head1 SYNOPSIS

Creation:

	use FinalBioIO::MyIO qw(getFh makeOutputDir);
	my $infile = 'test.txt';
...
=head1 DESCRIPTION	
 ...
 This module was designed for use by the final programs, and shows how to create a Perl a Perl package for file IO
...
sub makeOutputDir{
	...
	unless (-e $nameDir){ #do nothing if it already exists
		...
		if($@){ #alert user if directory cant be made
